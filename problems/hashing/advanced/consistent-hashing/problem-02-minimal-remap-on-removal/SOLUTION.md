# Removing a Node Remaps Only Its Keys — Solution

## Optimal Approach

The minimal-disruption property falls out of the ring structure: a removed node's positions vanish, so only keys that landed in those arcs move (to the next surviving node clockwise). Every other key's successor position is unchanged.
...
O(K log M) to snapshot + re-query K keys against M virtual nodes, plus O(vnodes log M) to remove and re-add the node.

## Key Insights & Edge Cases

The second component is ALWAYS True — that is the theorem. The count of moved keys is exactly the number that were owned by the removed server (about K/N on average), which is why consistent hashing is the backbone of Dynamo-style stores, memcached client rings, and CDN shard maps: scaling the fleet touches a small, bounded slice of keys.
