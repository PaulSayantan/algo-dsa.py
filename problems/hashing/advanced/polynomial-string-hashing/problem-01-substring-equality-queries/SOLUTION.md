# Substring Equality Queries — Solution
...
The subtraction can go negative before `% M`; Python's `%` returns a non-negative result for a positive modulus, so the formula is safe.
