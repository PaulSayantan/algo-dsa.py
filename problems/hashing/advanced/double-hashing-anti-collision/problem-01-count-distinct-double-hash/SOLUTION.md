# Count Distinct Substrings of Length L (Double Hash) — Solution
...
O(n) preprocessing per channel + O(n) window keys.

## Key Insights & Edge Cases

The tuple `(h1, h2)` is a deterministic dict/set key; we never call the salted built-in `hash()` on it.
